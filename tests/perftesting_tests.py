...
class TestPerfTesting(unittest.TestCase):
    ...
    def test_get_random_transactions(self):
        transactions = get_random_transactions(transaction_number=5, max_item_per_transaction=2, universe_size=10)
        self.assertEqual(len(transactions), 5)
        for trans in transactions:
            self.assertTrue(len(trans) <= 2)

    def test_sam(self):
        n, report = test_sam(ts=[('a', 'b'), ('b', 'c'), ('a', 'c')], support=2)
        self.assertEqual(n, len(report))
        self.assertTrue(frozenset(['a', 'b']) in report or frozenset(['b', 'c']) in report)

    def test_relim(self):
        n, report = test_relim(ts=[('a', 'b'), ('b', 'c'), ('a', 'c')], support=2)
        self.assertEqual(n, len(report))
        self.assertTrue(frozenset(['a', 'b']) in report or frozenset(['b', 'c']) in report)

    def test_fpgrowth(self):
        n, report = test_fpgrowth(ts=[('a', 'b'), ('b', 'c'), ('a', 'c')], support=2)
        self.assertEqual(n, len(report))
        self.assertTrue(frozenset(['a', 'b']) in report or frozenset(['b', 'c']) in report)

    def test_test_itemset_perf(self):
        # This is more of a performance test, so we might just run it and check it doesn't raise exceptions
        try:
            test_itemset_perf(perf_round=1, sparse=True, seed=42)
